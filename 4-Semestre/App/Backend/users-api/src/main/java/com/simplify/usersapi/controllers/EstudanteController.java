...
public class EstudanteController {
    ...
    public ResponseEntity postPilha(@RequestBody Estudante estudante){
        aux.push(estudante);
    ...
    public ResponseEntity post(@RequestBody Estudante user) {
    ...
    public ResponseEntity login(@RequestBody Login login){
        List<Estudante> users = repository.findAll();
        for (Estudante user: users){
            for (Integer i = 0; i < repository.count();i++){
                if (user.getEmail().equals(login.getEmail()) && user.getSenha().equals(login.getSenha())){
                    this.isSignin =true;
